...
public class Main {
	...
	public static void main(String[] args) {
		...
		for (OrderedItem oi:orderedItemList) {
			...
			String rate=oi.getRate();
			String qty=oi.getOrderQty();
			SnackItem s=ItemData.getItem(name);
			Double price=Double.parseDouble(s.getRate())*Double.parseDouble(qty);
			String dis=oi.getDiscountAmount();
			String amount=oi.getPayAmount();
			billAmount+= Double.parseDouble(amount);
			String output1=String.format("%-10s %-10s %-10s %-10s %-10s %-10s", name,rate,qty,price,dis,amount);
